...
import { getAdaptedEvent, getEventSurveys, type DbSurvey } from "@/lib/db";
import { type Event } from "@/lib/mock-data";
import { format } from "date-fns";
import {
  ArrowLeft, Star, ThumbsUp, ThumbsDown, Download,
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          key={n}
          className={`w-4 h-4 ${n <= value ? "fill-yellow-400 text-yellow-400" : "text-gray-200"}`}
        />
      ))}
    </div>
  );
...
export default function AdminSurveysPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    getEventSurveys(id).then((s) => { setSurveys(s); setLoading(false); }).catch(() => setLoading(false));
  }, [id]);
  ...
  const exportCSV = () => {
    const headers = [
      "Name", "Email", "Overall Rating", "Organization Rating",
      "Would Recommend", "Liked Most", "Could Improve", "Other Feedback", "Submitted At",
    ];
    const rows = surveys.map((s) => [
      s.student_name, s.student_email,
      s.rating, s.organization_rating,
              ...
              <div className="flex items-start justify-between gap-4 flex-wrap">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="text-xs text-gray-400">
                    {format(new Date(s.submitted_at), "dd MMM yyyy, h:mm a")}
                  </span>
                  {s.would_recommend ? (
